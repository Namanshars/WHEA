...
import com.yorku.wbapp.analysis.ConcreteClasses.AnalysisFour;
...
import com.yorku.wbapp.visualizer.*;
...
public class MainUI extends JFrame {
	...
	MainUI() {
		...
		//TODO: create FrontEndManagerInterface
		...
		AnalysisStrategy analysisOne = new AnalysisOne("CO2 vs EnergyUse vs PM2.5 Annual Percentage Change");
		...
		new LineVisualizer().createVisual(west, analyzedDataMap, null, analysisOne.getAnalysisName());
		new ScatterVisualizer().createVisual(west, analyzedDataMap, analyzedDataMap, analysisOne.getAnalysisName());
		new BarVisualizer().createVisual(west, analyzedDataMap, analyzedDataMap, analysisOne.getAnalysisName());
		new TimeSeriesVisualizer().createVisual(west, analyzedDataMap, analyzedDataMap, analysisOne.getAnalysisName());

		new ReportVisualizer().createVisual(west, analyzedDataMap, null, analysisOne.getAnalysisName());

		AnalysisStrategy analysisFour = new AnalysisFour(AnalysisConstants.FOREST_AREA);
		Map<String, WBData> wbDataMapPie = new HashMap<>();
		wbDataMapPie.put(AnalysisConstants.FOREST_AREA, co2Data);
		Map<String, WBData> analyzedDataMapPie = analysisFour.analyse(wbDataMapPie);
		new PieVisualizer().createVisual(west, analyzedDataMapPie, null, analysisFour.getAnalysisName());
